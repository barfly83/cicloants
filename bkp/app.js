...
class PheromoneEngine {
  ...
  deposit(points, quality = 1.0) {
    ...
    for (const { lat, lng } of points) {
      ...
      if (!merged) {
        this.pts.push({ lat, lng, intensity: dose, ts: Date.now() });
        added++;
      }
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Patch: aggiorna heatmap dopo ogni deposit()
  const _origDeposit = PheromoneEngine.prototype.deposit;
  PheromoneEngine.prototype.deposit = function (...args) {
    const result = _origDeposit.apply(this, args);
    // Solo se la mappa è già inizializzata
